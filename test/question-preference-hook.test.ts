...
const invocation = bashScriptInvocation(HOOK, []);
if (!invocation) throw new Error('Git Bash is required');
...
function runHook(stdin: object, cwd?: string, extraEnv?: Record<string, string>): {
  ...
  // Match the Codex runner's canonical identity across native and Git Bash Temp aliases.
  env.GSTACK_PROJECT_SLUG = path.basename(cwd || fixtureCwd);
...
describe('passes through (no enforcement)', () => {
  ...
  test('empty stdin → pass-through (crash safety)', () => {
    ...
    env.GSTACK_STATE_ROOT = stateRoot;
    const res = spawnSync(invocation.cmd, invocation.argv, { env, input: '', encoding: 'utf-8', timeout: 30_000 });
...
describe('Conductor spawned deny (#2733)', () => {
  ...
  test('spawnedByEnv() parity with bin/gstack-session-kind over the spawned env matrix', () => {
    ...
    for (const env of cases) {
      const scriptInvocation = bashScriptInvocation(BIN, []);
      if (!scriptInvocation) throw new Error('Git Bash is required');
      const scriptKind = spawnSync(scriptInvocation.cmd, scriptInvocation.argv, {
