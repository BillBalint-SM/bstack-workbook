...
// Extensionless Bash helpers must not be passed to cmd.exe on Windows.
function execBash(command: string, opts: ExecSyncOptions = {}): string {
  const bash = process.platform === 'win32' ? bashScriptInvocation('-s', [])!.cmd : 'bash';
  return execFileSync(bash, ['--noprofile', '--norc', '-s'], {
    ...opts, input: command, encoding: 'utf8', timeout: opts.timeout ?? 10000,
  });
}

const shellBin = BIN.replaceAll('\\', '/');
...
function run(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execBash(`"${shellBin}/gstack-review-log" '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
describe('gstack-review-log', () => {
  ...
  test('append still succeeds outside a git repo (binding fields omitted)', () => {
    ...
    try {
      ...
      execBash(`"${shellBin}/gstack-review-log" '{"skill":"review","status":"clean"}'`, execOpts);
...
describe('gstack-wtree', () => {
  function withScratchRepo(fn: (repoDir: string, wtree: () => string) => void) {
    ...
    try {
      ...
      git('commit -q -m init');
      const wtree = () => execBash(`"${shellBin}/gstack-wtree"`, { cwd: repoDir, encoding: 'utf-8', timeout: 10000 }).trim();
      fn(repoDir, wtree);
    } finally {
  ...
  test('racy-git window stays closed even when touch fails (stubbed-touch fallback)', () => {
    withScratchRepo((repoDir, _wtree) => {
      ...
      const wtreeStubbed = () =>
        execBash(`export PATH="${stubDir.replaceAll('\\', '/')}:$PATH"\n"${shellBin}/gstack-wtree"`, {
          cwd: repoDir,
          encoding: 'utf-8',
          timeout: 10000,
        }).trim();
  ...
  test('exits non-zero outside a git repo', () => {
    ...
    try {
      expect(() => execBash(`"${shellBin}/gstack-wtree"`, { cwd: nonGit, timeout: 10000, stdio: 'pipe' })).toThrow();
    } finally {
      fs.rmSync(nonGit, { recursive: true, force: true });
    }
  });
...
describe('gstack-review-read', () => {
  test('emits ---WTREE---, ---TREE--- and ---DIRTY--- sections', () => {
    const out = execBash(`"${shellBin}/gstack-review-read"`, {
      cwd: ROOT,
      env: { ...process.env, GSTACK_HOME: tmpDir },
      encoding: 'utf-8',
      timeout: 10000,
