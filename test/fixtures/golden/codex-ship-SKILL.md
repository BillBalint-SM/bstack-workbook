...
Use the available, authorized Codex subagent tools. If independent subagents
...
For each selected specialist, launch an independent subagent via the Codex subagent tool.
**Launch ALL selected specialists in a single message** (multiple Codex subagent tool calls)
...
- Use the host's documented spawn arguments; do not pass Claude-only options.
- Keep reviewer IDs and wait for ALL selected reviewers to finish before merging.
- Reviewers are read-only and must not delegate, edit, or approve user decisions.
...
If activated, dispatch one independent, read-only reviewer via the available Codex subagent tool and wait for its result before continuing. Apply the same authorization and availability requirements as specialist dispatch.
