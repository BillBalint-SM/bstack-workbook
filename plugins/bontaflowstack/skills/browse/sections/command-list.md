...
Set `B="$BFSTACK_BROWSE/browse.exe"` inside launcher `run`. Invoke it as `"$B"`; do not search profile or other installation directories.
...
| `eval <file> [--out <file>] [--raw]` | Run JavaScript from a file in the page context and return result as string. Path must resolve under the runtime temporary directory or cwd (no traversal). Use eval for multi-line scripts; use js for one-liners. With --out <file>, the result is written to disk (base64 data URL decoded to bytes unless --raw); --out makes the invocation a WRITE (requires local write scope). |
...
| `focus [@ref]` | OUT on Windows: do not promise window focusing through this command. |
...
| `status` | Report browser health; starts a headless daemon if none exists. For visible startup, run `connect` first. Do not use as a read-only preflight. |
