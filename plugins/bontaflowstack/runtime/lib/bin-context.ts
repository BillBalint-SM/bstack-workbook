/**
 * bin-context — tiny shared helpers for non-interactive bfstack bins that need the
 * project slug, current branch, and argv flags. Extracted from the decision bins
 * (bfstack-decision-log / bfstack-decision-search) so the slug/branch/flag plumbing
...
/** Keep the slug inside the [a-zA-Z0-9._-] alphabet bfstack-slug promises (`tr -cd`). */
 ...
 * `C:\work\example` → `/c/work/example`. bfstack-slug keys its cache on THAT form
 * (`tr '/' '_'`), so a native lookup must reproduce it exactly or it misses the very
 * entry bfstack-slug wrote and silently re-derives instead of staying consistent.
...
// Marker tiers mirror bin/bfstack-slug's `_outermost_project_root` exactly.
...
/**
 * Native port of bin/bfstack-slug's `_outermost_project_root`: walk UP
...
/**
 * Native port of bin/bfstack-slug's `_outermost_remote_repo` (step 1a): walk UP
...
/**
 * Native port of bin/bfstack-slug's resolution order, used when that script cannot be
 ...
 * test/bin-context-windows-slug.test.ts against test/bfstack-slug-cwd-walk-up.test.ts
 * and test/bfstack-slug-parity.test.ts):
 *   0. $BFSTACK_PROJECT_SLUG env override — wins over everything, never cached.
 ...
 *      like `url = ..` must never escape ~/.bfstack/projects/<slug>.
 *   4. Project root's basename; else basename(cwd) for plain non-project folders.
 */
export function slugFromEnvironment(bfstackHome?: string, cwd: string = process.cwd()): string {
  const home = bfstackHome || process.env.BFSTACK_HOME || join(homedir(), ".bfstack");
  ...
  const envSlug = sanitizeSlug((process.env.BFSTACK_PROJECT_SLUG || "").trim());
  if (envSlug) return envSlug;

  // 1. outermost project root along the cwd ancestor chain (may be "").
  const projectRoot = outermostProjectRoot(cwd);

  // Lazy, memoized remote discovery (mirrors bfstack-slug's _resolve_remote):
  ...
  // 3. canonical remote-derived slug from the outermost remote-bearing repo.
  //    Parse mirrors bin/bfstack-slug step 2 exactly (byte-parity with
  ...
  if (!slug) {
    ...
    if (url) {
      ...
      // Dot-only / degenerate guard (mirrors bin/bfstack-slug): a hostile
      // origin like `url = ..` yields "." or ".." here, which would file
      // state OUTSIDE ~/.bfstack/projects/. Reject and let the basename
  ...
  // 5. cache it, as bfstack-slug does — atomic, self-healing (only rewrites when
 ...
 *  PATHEXT match for an explicit path), so bfstack-slug spawns ENOENT there. */
export const NEEDS_NATIVE_SLUG_ON_WINDOWS = process.platform === "win32";

/**
 * Resolve the project slug via the `bfstack-slug` helper (parses `SLUG=...`).
 *
 * On Windows that spawn fails ENOENT (see NEEDS_NATIVE_SLUG_ON_WINDOWS) and `r.stdout`
 * is undefined — a command may otherwise resolve through an unintended shell shim.
 * (#1731). Returning the literal "unknown" filed every decision under
 * ~/.bfstack/projects/unknown/ — one bucket shared by every project on the machine —
