/**
 * jsonl-store — shared plumbing for bfstack's append-only JSONL stores in
 ...
 *      today: bin/bfstack-learnings-log, bin/bfstack-decision-log (via
 *      lib/bfstack-decision.ts), bin/bfstack-question-log.
 ...
 *
 * Extracted from `bin/bfstack-learnings-log` (D2A) so the learnings/decision/
...
export const INJECTION_PATTERNS: readonly RegExp[] = [
  ...
  /\bassistant\s*:/i,
  /\buser\s*:/i,
  /\bhuman\s*:/i, // model native turn prefix — bypassed the denylist AND datamark
