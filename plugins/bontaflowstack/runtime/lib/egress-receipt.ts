 ...
 * bfstack-initiated off-machine send (`~/.bfstack/security/egress.jsonl`, 0600).
 *
...
export interface WriteReceiptOptions {
  /** bfstack home; resolved from env when omitted */
  home?: string;
  /** env for home resolution (tests) */
  env?: Env;
  /** which bfstack component is sending */
  sink: string;
   ...
   * Callers on an interactive hot path with their own deadline (a the host
...
/**
 * Same resolution order as the rest of bfstack (shell sinks, selection code):
 * BFSTACK_HOME, legacy BFSTACK_STATE_DIR, then $HOME/.bfstack.
 */
export function resolveEgressHome(env: Env = process.env): string {
  const configured = env.BFSTACK_HOME || env.BFSTACK_STATE_DIR;
  if (configured) return path.resolve(configured);
  return path.join(env.HOME || os.homedir(), '.bfstack');
}
 ...
 * (minutes apart; some interactive events can be bursty); the lock
 ...
 * bfstack ATTEMPTS to send off-machine), how to inspect it, and that
 * trimming arrives with rotation.
 */
export function ledgerSizeWarning(ledger: string, size: number): string {
  const mb = (size / (1024 * 1024)).toFixed(1);
  return (
    `bfstack: egress ledger is large (${mb}MB): ${ledger}. ` +
    `This file records what bfstack ATTEMPTS to send off-machine (content-free receipts, for auditing). ` +
    `Inspect it with 'bfstack-egress list'. Trimming arrives with ledger rotation (TODO); until then it only grows.`
  );
}
