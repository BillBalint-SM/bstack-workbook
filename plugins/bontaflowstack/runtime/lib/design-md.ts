...
//        ──► convertLegacy  ──► bfstack's pre-spec DESIGN.md (Product Context, Aesthetic Direction,
...
//        ──► upsertSection  ──► body-only splice on the parsed doc (files bfstack writes from scratch)
...
//   spec files:   line 1 `---`, line 2 `# bfstack: design-md-format=spec` (a YAML comment, so
//                 parsers that require `---` on line 1 keep working)
//   legacy files: line 1 `<!-- bfstack: design-md-format=legacy-keep -->`
...
export const FORMAT_MARKER_PREFIX = 'bfstack: design-md-format=';
...
/** `<!-- bfstack: design-md-format=... -->` on line 1 (legacy files) */
const LEGACY_MARKER_RE = new RegExp('^<!--[ \\t]*' + MARKER_RE_BODY + '[ \\t]*-->\\n?');
/** `# bfstack: design-md-format=...` as a YAML comment inside the front matter (spec files) */
...
/** Headings that mark bfstack's pre-spec file by themselves (either one is enough evidence of a legacy shape). */
...
/** Headings that identify bfstack's pre-spec DESIGN.md. */
...
export function isLegacyBfstackFormat(doc: DesignMdDoc): boolean {
...
export function detectFormat(doc: DesignMdDoc | null): { format: DesignMdFormat; code: FormatCode; reason?: string } {
  ...
  if (isLegacyBfstackFormat(doc)) return { format: 'legacy', code: 'legacy' };
  ...
  return { format: 'unknown', code: 'no-shape', reason: 'no front matter and no bfstack legacy headings' };
}
 ...
 * that edits a file the user owns should use; renderDesignMd is for files bfstack
...
/**
 * Convert bfstack's pre-spec DESIGN.md into the open format. Product Context and
