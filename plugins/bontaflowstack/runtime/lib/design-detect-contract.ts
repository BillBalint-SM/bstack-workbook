...
// (bin/bfstack-design-detect.ts) or the DESIGN.md tool (bin/bfstack-design-md.ts)
...
//   scan  ──► stdout:  one JSON document (--format bfstack) or engine bytes (--format raw)
...
//   any   ──► exit 3 + DESIGN_DETECT_INTERNAL_ERROR: a bfstack bug, never retried
...
export const SENTINEL = {
  ...
  /** printed by bfstack-design-md check / convert */
...
/**
 * Checksums bfstack pins for the engine versions it has tested, per platform:
 ...
 * linux-x64 hash also matches the engine bfstack's fixtures were captured with.
...
export const DETECT_LIMITS = {
  /** default engine wall clock; BFSTACK_DESIGN_DETECT_TIMEOUT_MS overrides */
  timeoutMs: 120_000,
...
/**
 * One pass over every shape the agent reads as bfstack's own voice: the two fence
