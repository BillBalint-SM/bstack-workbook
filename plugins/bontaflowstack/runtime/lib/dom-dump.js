() => {
  ...
  liveLinks.forEach((link, i) => {
    ...
    try {
      ...
      inlined.push("/* bfstack-dom-dump: " + (sheet.href || "link") + " */\n" + text);
  ...
  const cleanCss = (t) => t.replace(dataUrl, "url(data:,bfstack-stripped)").replace(cssQuery, "url($1$2$1)");
  if (inlined.length) {
    const style = document.createElement("style");
    style.setAttribute("data-bfstack-dom-css", "");
  ...
  for (const el of Array.from(root.querySelectorAll("style"))) {
    if (el.getAttribute("data-bfstack-dom-css") === null && el.textContent) el.textContent = cleanCss(el.textContent);
  }
  ...
  for (const el of Array.from(root.querySelectorAll("*"))) {
    for (const attr of Array.from(el.attributes)) {
      ...
      else if (value.indexOf("data:") === 0 && value.length > 1024) el.setAttribute(name, "data:,bfstack-stripped");
    }
  }
