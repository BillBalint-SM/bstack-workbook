/** BontaFlowStack browser sidebar: local actions, activity, refs and inspection. */
...
function setConnState(state) {
  ...
  if (state === 'connected') {
    if (status) status.textContent = 'Connected to local browser';
    ...
    if (status) status.textContent = 'Browser disconnected';
    banner.style.display = 'none';
  }
}
...
// The browser pane is the only primary surface; Activity / Refs / Inspector
// are debug overlays behind the `debug` toggle. Closing debug returns to
// the browser pane, which is always present.
const PRIMARY_PANE_ID = 'tab-home';
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[bfstack sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[bfstack sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[bfstack sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function pollMemoryOnce() {
  ...
  try {
    ...
    try { evaluateMemToast(snapshot); } catch (err) {
      console.debug('[bfstack sidebar] mem-toast evaluation failed:', err && err.message);
    }
    ...
    if (elapsed > MEM_POLL_SLOW_THRESHOLD_MS) {
      console.debug('[bfstack sidebar] /memory poll slow/failed:', elapsed, 'ms', err && err.message);
    }
...
async function loadSnoozeState() {
  ...
  try {
    ...
    console.debug('[bfstack sidebar] mem-toast snooze load failed:', err && err.message);
  }
}
...
async function saveSnoozeState() {
  ...
  try {
    ...
    console.debug('[bfstack sidebar] mem-toast snooze save failed:', err && err.message);
  }
}
  ...
  closeBtn.onclick = async () => {
    ...
    for (const id of ids) {
      try {
        ...
        console.warn('[bfstack sidebar] mem-toast closetab failed:', id, err && err.message);
      }
    }
  };
}
  ...
  if (snooze) {
    snooze.addEventListener('click', async () => {
      ...
      try {
        ...
        console.debug('[bfstack sidebar] mem-toast snooze fetch failed:', err && err.message);
      }
      dismissMemToast();
    });
  }
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[bfstack sidebar] Failed to fetch refs:', err.message);
  }
}
...
function inspectorShowError(message) {
  ...
  inspectorSelected.textContent = '';
  inspectorSelected.title = '';
  ...
  function commit() {
    ...
    if (newVal && newVal !== currentVal) {
      chrome.runtime.sendMessage({
        ...
        property: prop,
        value: newVal,
        target: inspectorData?.target || null,
      });
      inspectorModifications.push({ property: prop, value: newVal, selector, target: inspectorData?.target || null });
      updateSendButton();
    }
  }
...
function updateSendButton() {
  if (inspectorModifications.length > 0) {
    inspectorSendBtn.textContent = 'Copy modifications';
    inspectorSendBtn.title = `Copy ${inspectorModifications.length} modification(s) for your Codex task`;
  } else {
    inspectorSendBtn.textContent = 'Copy inspector data';
    inspectorSendBtn.title = 'Copy inspector data for your Codex task';
  }
}
...
inspectorSendBtn.addEventListener('click', async () => {
  ...
  if (inspectorModifications.length > 0) {
    ...
    // Copy inspector data for your Codex task
    message = `CSS Inspector data for: ${inspectorData.selector}\n\n${JSON.stringify(inspectorData, null, 2)}`;
  }

  try {
    await navigator.clipboard.writeText(message);
    inspectorSendBtn.textContent = inspectorModifications.length > 0 ? 'Copied modifications' : 'Copied inspector data';
    setActionStatus('Inspector data copied to the clipboard.', 'success');
  } catch (error) {
    console.error('[browser] Could not copy inspector data:', error.message);
    setActionStatus(`Could not copy inspector data: ${error.message}`, 'error');
    updateSendButton();
  }
});
...
const pendingQuickActions = new Set();

function setActionStatus(message, kind = '') {
  const status = document.getElementById('action-status');
  if (!status) return;
  status.textContent = message;
  status.className = `action-status${kind ? ` ${kind}` : ''}`;
}

function setQuickActionState(buttons, loading) {
  buttons.forEach(button => {
    if (!button) return;
    button.classList.toggle('loading', loading);
    button.disabled = loading || connState !== 'connected';
  });
}

function responseMessage(responseText, fallback) {
  const message = responseText.trim();
  if (!message) throw new Error(fallback);
  return message;
}

async function runQuickAction(command, args, buttons, successPrefix) {
  if (pendingQuickActions.has(command)) return;
  if (!serverUrl || !serverToken || connState !== 'connected') {
    setActionStatus('Browser connection is unavailable.', 'error');
    return;
  }
  pendingQuickActions.add(command);
  setQuickActionState(buttons, true);
  try {
    ...
    if (!response.ok) throw new Error(responseMessage(text, `${successPrefix} failed.`));
    const message = responseMessage(text, `${successPrefix} completed.`);
    setActionStatus(message.startsWith(`${successPrefix}:`) ? message : `${successPrefix}: ${message}`, 'success');
  } catch (error) {
    console.error(`[browser] ${command} failed:`, error.message);
    setActionStatus(`${successPrefix} failed: ${error.message}`, 'error');
  } finally {
    pendingQuickActions.delete(command);
    setQuickActionState(buttons, false);
  }
}

async function runCleanup(...buttons) {
  return runQuickAction('cleanup', ['--all'], buttons, 'Clean page complete');
}

async function runScreenshot(...buttons) {
  return runQuickAction('screenshot', [], buttons, 'Screenshot saved');
}
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[bfstack sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[bfstack sidebar] Inspector SSE not available:', err.message);
  }
}
...
async function updateConnection(url, token) {
  if (url && token) {
    try {
      const health = await fetch(`${url}/health`, { signal: AbortSignal.timeout(2000) });
      if (!health.ok) throw new Error(`health returned ${health.status}`);
    } catch (error) {
      console.warn('[bfstack sidebar] Browser health check failed:', error.message);
      url = null;
      token = null;
    }
  } else if (url) {
  ...
  // Keep the local authenticated browser connection available to sidebar actions.
  if (url) {
    try { window.bfstackServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.bfstackAuthToken = token || null;
  } else {
    window.bfstackServerPort = null;
    window.bfstackAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-bfstack-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-bfstack-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'bfstack-migration-notice' && msg.message) {
    console.log('[bfstack sidebar]', msg.message);
    try {
      const banner = document.createElement('div');
      ...
      console.debug('[bfstack sidebar] migration banner failed:', err && err.message);
    }
  }
  if (msg.type === 'inspectResult') {
    ...
    inspectorPickBtn.classList.remove('active');
    if (msg.data?.error) {
      inspectorShowError(msg.data.error);
    } else if (msg.data) {
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('bfstack:tab-state', {
