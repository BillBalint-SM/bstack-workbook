...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'bfstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="bfstack-pill-dot"></span> bfstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'bfstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'bfstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'bfstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'bfstack-ref-panel-header';
  header.textContent = `bfstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'bfstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'bfstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'bfstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'bfstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'bfstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'bfstack-ref-panel-more';
...
// Do NOT dispatch bfstack-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('bfstack-extension-ready'));
    return;
  }
