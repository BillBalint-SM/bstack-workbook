/**
 * bfstack browse — background service worker
 *
...
// Classic (non-module) service worker: importScripts puts bfstackSenderAuth on
...
async function loadAuthToken() {
  ...
  try {
    ...
    if (resp.status === 403) {
      console.error('[bfstack bg] /extension-token 403 — extension identity not trusted by server');
      authToken = null;
      setDisconnected();
      return false;
    }
    ...
    console.error('[bfstack bg] Failed to load auth token:', err.message);
  }
  return false;
}
...
async function checkHealth() {
  ...
  try {
    ...
    console.error('[bfstack bg] Health check failed:', err.message);
    setDisconnected();
  }
}
...
function setConnected(healthData) {
  ...
  chrome.runtime.sendMessage({ type: 'health', data: healthData }).catch((err) => {
    console.debug('[bfstack bg] No listener for health broadcast:', err.message);
  });
...
function setDisconnected() {
  ...
  chrome.runtime.sendMessage({ type: 'health', data: null }).catch((err) => {
    console.debug('[bfstack bg] No listener for disconnect broadcast:', err.message);
  });
...
async function notifyContentScripts(type) {
  try {
    ...
    console.error('[bfstack bg] Failed to query tabs for notification:', err.message);
  }
}
...
async function fetchAndRelayRefs() {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn(`[bfstack bg] Refs endpoint returned ${resp.status}`);
      return;
    }
    const data = await resp.json();
    ...
    console.error('[bfstack bg] Failed to fetch/relay refs:', err.message);
  }
}
...
async function injectInspector(tabId) {
  ...
  try {
    ...
    try {
      ...
      console.debug('[bfstack bg] Inspector CSS injection failed (non-fatal):', err.message);
    }
    // Dispatch in every injected frame. tabs.sendMessage() alone addresses
    // frame 0, which silently left iframe picks inactive.
    try {
      await chrome.scripting.executeScript({
        target: { tabId, allFrames: true },
        func: () => window.dispatchEvent(new Event('bfstack-inspector-start')),
      });
    } catch (err) {
      console.warn('[bfstack bg] Failed to send startPicker:', err.message);
    }
    inspectorMode = 'full';
    ...
    try {
      ...
      console.error('[bfstack bg] Inspector injection failed completely:', err.message, '| Basic fallback:', err2.message);
      inspectorMode = 'full';
...
async function stopInspector(tabId) {
  try {
    await chrome.scripting.executeScript({
      target: { tabId, allFrames: true },
      func: () => window.dispatchEvent(new Event('bfstack-inspector-stop')),
    });
  } catch (err) {
    console.debug('[bfstack bg] Failed to stop picker on tab', tabId, ':', err.message);
  }
  return { ok: true };
}

async function postInspectorPick(selector, frameInfo, basicData, target) {
  const base = getBaseUrl();
  if (!base || !authToken) {
    // No browse server — return basic data as fallback
    return { mode: 'basic', selector, basicData, frameInfo, target };
  }
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      // A rejected target must stay visible; basic data would make a stale or
      // ambiguous selection look valid.
      return { error: error.error || 'Inspector target is no longer available', target };
    }
    const data = await resp.json();
    return { mode: data.inspectionMode || 'cdp', ...data };
  } catch (err) {
    console.debug('[bfstack bg] Inspector pick server unavailable, using basic mode:', err.message);
    return { mode: 'basic', selector, basicData, frameInfo, target };
  }
}

async function sendToContentScript(tabId, message, target) {
  try {
    const options = {};
    if (Number.isInteger(target?.chromeFrameId)) options.frameId = target.chromeFrameId;
    if (typeof target?.chromeDocumentId === 'string') options.documentId = target.chromeDocumentId;
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (sender.id !== chrome.runtime.id) {
    console.warn('[bfstack] Rejected message from unknown sender:', sender.id);
    return;
  }
  ...
  if (!ALLOWED_TYPES.has(msg.type)) {
    console.warn('[bfstack] Rejected unknown message type:', msg.type);
    return;
  }
  ...
  const denial = bfstackSenderAuth.denialFor(msg.type, sender, chrome.runtime.id);
  if (denial) {
    console.warn('[bfstack] Rejected privileged message from unauthorized sender:', msg.type, sender.url || '(no sender url)');
    sendResponse(denial);
    return true;
  }
  ...
  if (msg.type === 'openSidePanel') {
    if (chrome.sidePanel?.open && sender.tab) {
      chrome.sidePanel.open({ tabId: sender.tab.id }).catch((err) => {
        console.warn('[bfstack bg] Failed to open side panel:', err.message);
      });
    }
    return;
  }
  ...
  if (msg.type === 'elementPicked') {
    ...
    if (!Number.isInteger(chromeTabId) || !sender.url) {
      sendResponse({ error: 'Inspector pick has no stable source target' });
      return;
    }
    const frameInfo = msg.frameSrc ? { frameSrc: msg.frameSrc, frameName: msg.frameName } : null;
    const target = {
      documentUrl: sender.tab.url || sender.url,
      documentToken: msg.documentToken,
      ...(msg.frameSrc ? { frameUrl: msg.frameSrc, frameName: msg.frameName || null, frameDocumentToken: msg.frameDocumentToken } : {}),
      chromeTabId,
      chromeFrameId: sender.frameId,
      ...(typeof sender.documentId === 'string' ? { chromeDocumentId: sender.documentId } : {}),
    };
    postInspectorPick(msg.selector, frameInfo, msg.basicData, target)
        .then(result => {
          if (result.target) {
            result.target = { ...result.target, chromeTabId, chromeFrameId: sender.frameId, ...(typeof sender.documentId === 'string' ? { chromeDocumentId: sender.documentId } : {}) };
          }
          ...
          chrome.runtime.sendMessage({
            ...
            console.warn('[bfstack bg] Failed to forward inspectResult to sidepanel:', err.message);
          });
          sendResponse({ ok: true });
        });
    return true;
  ...
  if (msg.type === 'pickerCancelled') {
    chrome.runtime.sendMessage({ type: 'pickerCancelled' }).catch((err) => {
      console.debug('[bfstack bg] No listener for pickerCancelled:', err.message);
    });
    return;
  }
  ...
  if (msg.type === 'applyStyle' || msg.type === 'toggleClass' || msg.type === 'injectCSS' || msg.type === 'resetAll') {
    ...
    if (!target || !Number.isInteger(target.chromeTabId)) {
      sendResponse({ error: 'Inspector target is missing or stale. Pick the element again.' });
      return;
    }
    sendToContentScript(target.chromeTabId, msg, target).then(result => sendResponse(result));
    return true;
  }
...
if (chrome.sidePanel && chrome.sidePanel.setPanelBehavior) {
  chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true }).catch((err) => {
    console.warn('[bfstack bg] Failed to set panel behavior:', err.message);
  });
}
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (wins.length > 0) {
        ...
        console.log(`[bfstack] Side panel opened on attempt ${attempt + 1}`);
        return; // success
      }
    } catch (e) {
      // May throw if window isn't ready or user gesture required
      console.log(`[bfstack] Side panel open attempt ${attempt + 1} failed:`, e.message);
    }
    // Backoff: 500ms, 1000ms, 2000ms, 3000ms, 5000ms
    await new Promise(r => setTimeout(r, [500, 1000, 2000, 3000, 5000][attempt]));
  }
  console.log('[bfstack] Side panel auto-open failed after 5 attempts');
}
...
// ─── Tab Awareness ───────────────────────────────────────────────
// Push browser tab state to the sidebar for its local tab navigation.
...
chrome.tabs.onActivated.addListener((activeInfo) => {
  ...
  // unified state push so the sidebar reflects the new active tab.
...
chrome.tabs.onUpdated.addListener((_id, changeInfo) => {
  ...
  // tick. We don't want to spam the sidebar with a state push every 50ms while
  // a page loads.
...
async function announceIdentityPinOnce() {
  try {
    const data = await chrome.storage.local.get('bfstack_id_pin_migrated');
    if (data.bfstack_id_pin_migrated) return;
    console.log('[bfstack] bfstack sidebar: extension identity pinned in v1.63 — panel state reset once.');
    chrome.runtime.sendMessage({
      type: 'bfstack-migration-notice',
      message: 'bfstack sidebar: extension identity pinned in v1.63 — panel state reset once.',
    }).catch(() => {
      // Expected: panel not open. The console line above still lands.
    });
    await chrome.storage.local.set({ bfstack_id_pin_migrated: true });
  } catch (err) {
    console.debug('[bfstack] identity-pin notice failed (non-fatal):', err.message);
  }
}
...
loadPort().then(() => {
  ...
  const startupCheck = setInterval(async () => {
    ...
    if (isConnected || startupAttempts >= 15) {
      ...
      if (!isConnected) {
        console.log('[bfstack] Startup health checks failed after 15 attempts, falling back to 10s polling');
      }
    }
  }, 1000);
});
