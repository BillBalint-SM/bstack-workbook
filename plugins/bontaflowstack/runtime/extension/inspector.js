/**
 * bfstack browse — CSS Inspector content script
 *
...
(() => {
  ...
  if (window.__bfstackInspectorActive) return;
  window.__bfstackInspectorActive = true;
  ...
  const originalStyles = new Map(); // element -> Map<property, { value, priority }>
  ...
  function createHighlight() {
    ...
    highlightEl.id = 'bfstack-inspector-highlight';
    ...
    tooltipEl.id = 'bfstack-inspector-tooltip';
  ...
  function onMouseMove(e) {
    ...
    if (target.id === 'bfstack-inspector-highlight' || target.id === 'bfstack-inspector-tooltip') return;
  ...
  function onClick(e) {
    ...
    // Frame detection
    const frameInfo = {};
    const documentToken = String(performance.timeOrigin);
    if (window !== window.top) {
      try {
        frameInfo.frameSrc = window.location.href;
        frameInfo.frameName = window.name || null;
        frameInfo.frameDocumentToken = documentToken;
      } catch (e) { if (!(e instanceof DOMException)) throw e; }
    }
    ...
    chrome.runtime.sendMessage({
      ...
      basicData,
      documentToken: window === window.top ? documentToken : undefined,
      ...frameInfo,
    });
  ...
  // Background dispatches these DOM events into every frame because Chrome's
  // tabs.sendMessage defaults to the top frame.
  window.addEventListener('bfstack-inspector-start', startPicker);
  window.addEventListener('bfstack-inspector-stop', stopPicker);
  ...
  function applyStyle(selector, property, value) {
    ...
    if (!origMap.has(property)) {
      origMap.set(property, { value: el.style.getPropertyValue(property), priority: el.style.getPropertyPriority(property) });
    }
  ...
  function injectCSS(id, css) {
    ...
    const styleId = `bfstack-inject-${id}`;
  ...
  function resetAll() {
    ...
    for (const [el, propMap] of originalStyles) {
      for (const [prop, original] of propMap) {
        if (original.value) {
