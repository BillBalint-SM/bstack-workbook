 ...
 * <bfstack-qid:...> wrapper) when present, else a hash-based hook- id.
 ...
 * AUQ tool_response shape varies by normalized question-response schema,
...
function detectSkill(cwd: string | undefined): string {
  ...
  // skill marker (<bfstack-skill:NAME>) embedded in question text per
...
function spawnLog(payload: Record<string, unknown>, cwd?: string): void {
  const res = runBin('bfstack-question-log', [JSON.stringify(payload)], {
    encoding: 'utf-8',
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 3000,
    // Run from the originating tool call's cwd so bfstack-slug resolves to
    // the project the user is actually in, not the hook script's location.
    cwd: cwd && fs.existsSync(cwd) ? cwd : undefined,
  });
  if (res.status !== 0) {
    logHookError(`bfstack-question-log exited ${res.status}: ${res.stderr || res.stdout}`);
  }
}
