/**
 * Windows-safe resolution + spawn for bfstack's bash bins. Two Windows-only
 * bugs made every hook subprocess a silent no-op; both are fixed here so all
 * call sites are covered at once.
 *
 * 1. `new URL(import.meta.url).pathname` yields `/C:/Example/...`; path.resolve
 *    then rebases it onto the drive root as `C:\C:\Example\...`. fileURLToPath
 *    is the correct conversion. (ENOENT before the bin ever ran.)
 * 2. `bin/bfstack-*` are extensionless bash scripts. Windows has no shebang
...
function bashExe(): string {
  return process.env.BFSTACK_BASH || (fs.existsSync(GIT_BASH) ? GIT_BASH : 'bash');
}

/** bfstack install root. This file lives at hosts/codex/hooks/. */
...
/**
 * Run an EXTERNAL executable (not a bfstack bin) with containment a hook can
 * rely on:
