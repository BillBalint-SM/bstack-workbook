 ...
 * and passes it via BFSTACK_SKILL_TOKEN. The skill never sees the daemon root
...
async function handleTest(args: string[], ctx: SkillCommandContext): Promise<string> {
  ...
  const { stdout, stderr, exitCode } = await runToFiles([resolveBunExecutable(), 'test', testFile], {
    cwd: skill.dir,
    env: buildChildEnv(skill.frontmatter.trusted),
  });
 ...
 * BFSTACK_PORT, so a synchronous wait would deadlock it.
 */
async function runToFiles(cmd: string[], opts: RunToFilesOptions): Promise<RunToFilesResult> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'bfstack-skill-'));
  const outPath = path.join(dir, 'stdout');
  const errPath = path.join(dir, 'stderr');
  const useNodeFallback = !process.versions.bun;
  const outFd = useNodeFallback ? fs.openSync(outPath, 'w') : undefined;
  const errFd = useNodeFallback ? fs.openSync(errPath, 'w') : undefined;
  try {
    ...
    const proc = Bun.spawn(cmd, {
      ...
      cwd: opts.cwd,
      env: opts.env as any,
      stdout: useNodeFallback ? outFd : Bun.file(outPath) as any,
      stderr: useNodeFallback ? errFd : Bun.file(errPath) as any,
    });
 ...
 *    BFSTACK_PORT and BFSTACK_SKILL_TOKEN are always set.
...
export async function spawnSkill(opts: SpawnSkillOptions): Promise<SpawnSkillResult> {
  ...
  try {
    ...
    return await runToFiles([resolveBunExecutable(), 'run', scriptPath, '--', ...opts.skillArgs], {
      cwd: opts.skill.dir,
      env,
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get BFSTACK_PORT +
 * BFSTACK_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  const out = buildChildEnv(opts.trusted);

  // Inject the daemon connection (always last so callers can't override).
  out.BFSTACK_PORT = String(opts.port);
  out.BFSTACK_SKILL_TOKEN = opts.skillToken;

  return out;
}

function buildChildEnv(trusted: boolean): Record<string, string> {
  ...
  if (trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'BFSTACK_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip BFSTACK_TOKEN
  // above.)
  if (!trusted) {
...
function resolveMinimalPath(): string {
  ...
  return path.isAbsolute(executable)
    ? `${path.dirname(executable)}${path.delimiter}${fallback}`
    : fallback;
}

function resolveBunExecutable(): string {
  const bundled = process.env.BFSTACK_ROOT
    ? path.join(process.env.BFSTACK_ROOT, 'tools', process.platform === 'win32' ? 'bun.exe' : 'bun')
    : '';
  if (bundled && fs.existsSync(bundled)) return bundled;
  const current = process.execPath;
  if (/^bun(?:\.exe)?$/i.test(path.basename(current))) return current;
