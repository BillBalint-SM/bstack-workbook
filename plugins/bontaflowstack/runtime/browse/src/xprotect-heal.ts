 ...
 *      a BFSTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
 *      never touched (same scope contract as probePoisonedChromiumBundle).
 *   3. Force-reinstall Chromium FROM THE BFSTACK INSTALL ROOT (ENG-OV3: the
...
/**
 * Find the bfstack install root whose node_modules pins the SAME
 ...
 */
export function findBfstackInstallRoot(
  expectedRevision: string,
  candidates?: string[],
): string | null {
  const roots = candidates ?? [process.env.BFSTACK_ROOT || path.resolve(__dirname, '..', '..')];
 ...
 * on a BFSTACK_CHROMIUM_PATH bundle — that belongs to the wrapper/embedder.
...
/**
 * Run `bunx playwright install --force chromium` from the bfstack install
...
export async function maybeHealXProtectKill(
  ...
  if (opts.usesCustomExecutable) {
    // A BFSTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder — never
  ...
  const root = (deps.installRoot ?? findBfstackInstallRoot)(revision);
  if (!root) {
...
export function buildXProtectGuidance(originalMessage: string): string {
  return (
    ...
    'Fix manually: run `bunx playwright install chromium` from your bfstack install ' +
    '(the selected package directory whose node_modules pins playwright, for ' +
    'a supported development build), then retry.'
  );
}
