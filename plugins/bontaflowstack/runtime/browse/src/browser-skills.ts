 ...
 *   project:  <project>/.bfstack/browser-skills/<name>/
 *   global:   ~/.bfstack/browser-skills/<name>/
 *   bundled:  <bfstack-install>/browser-skills/<name>/   (read-only, ships with bfstack)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  // `home` remains a user-home override for existing standalone callers;
  // packaged launches supply BFSTACK_HOME and never read a second install's state.
  const bfstackHome = opts.home ? path.join(opts.home, '.bfstack') : resolveBfstackHome();
  const projectRoot = opts.projectRoot ?? detectProjectRoot();
  const bundledRoot = opts.bundledRoot ?? detectBundledRoot();

  return {
    project: projectRoot ? path.join(projectRoot, '.bfstack', 'browser-skills') : null,
    global: path.join(bfstackHome, 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <bfstack-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <bfstack-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the bfstack root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with bfstack and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the bfstack source tree.
 */
