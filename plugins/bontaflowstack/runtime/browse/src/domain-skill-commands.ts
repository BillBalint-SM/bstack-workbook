 ...
 *     suggested-action. Matches the bfstack house style.
 *
...
function formatSavedOk(row: DomainSkillRow, slug: string): string {
  return [
    ...
    `Version: ${row.version}`,
    `Stored at: ~/.bfstack/projects/${slug}/learnings.jsonl`,
    '',
...
async function handleEdit(args: string[]): Promise<string> {
  ...
  const tmpFile = path.join(os.tmpdir(), `bfstack-domain-skill-${process.pid}-${Date.now()}.md`);
...
async function handlePromoteToGlobal(args: string[]): Promise<string> {
  ...
  return [
    ...
    `Stored at: ~/.bfstack/global-domain-skills.jsonl`,
    '',
