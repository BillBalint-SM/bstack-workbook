...
//    write-commands.ts/meta-commands.ts → browser command handlers
...
if (!globalThis.Bun) globalThis.Bun = {
  ...
  spawn(cmd, options = {}) {
    const [command, ...args] = cmd;
    const output = (value) => value === undefined ? 'pipe' : value;
    const stdio = options.stdio || [options.stdin || 'pipe', output(options.stdout), output(options.stderr)];
    ...
    // discard further bytes. Override via BFSTACK_SPAWN_MAX_BUFFER (bytes).
    const MAX_BUFFER = Math.max(
      0,
      parseInt(process.env.BFSTACK_SPAWN_MAX_BUFFER || '', 10) || 16 * 1024 * 1024,
    );
    const drain = (stream) => {
