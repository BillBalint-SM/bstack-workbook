...
export interface StyleModification {
  ...
  targetKey?: string;
  /** Exact inline declaration state for a target-scoped inline mutation. */
  oldInlineValue?: string;
  oldInlinePriority?: string;
}
...
export async function inspectElement(
  page: Page,
  selector: string,
  options?: { includeUA?: boolean; session?: any; nodeId?: number }
): Promise<InspectorResult> {
  const session = options?.session || await getOrCreateSession(page);

  let nodeId = options?.nodeId;
  if (!nodeId) {
    // Get document root and query the main document.
...
async function resolveFrameNodeId(page: Page, frame: Frame, selector: string): Promise<{ session: any; nodeId: number }> {
  const session = await getOrCreateSession(page);
  const expectedToken = await frame.evaluate(() => String(performance.timeOrigin));
  const expectedUrl = frame.url();
  const expectedName = frame.name();
  const candidates: Array<{ id: string; url: string; name?: string }> = [];
  const failures: string[] = [];
  const collect = (node: any) => {
    if (node.frame) candidates.push(node.frame);
    for (const child of node.childFrames || []) collect(child);
  };
  collect((await session.send('Page.getFrameTree')).frameTree);

  for (const candidate of candidates) {
    if (candidate.url !== expectedUrl || (expectedName && candidate.name !== expectedName)) continue;
    try {
      const world = await session.send('Page.createIsolatedWorld', {
        frameId: candidate.id,
        worldName: 'bfstack-inspector',
        grantUniveralAccess: false,
      });
      const tokenResult = await session.send('Runtime.evaluate', {
        expression: 'String(performance.timeOrigin)',
        contextId: world.executionContextId,
        returnByValue: true,
      });
      if (tokenResult.result.value !== expectedToken) continue;
      const documentResult = await session.send('Runtime.evaluate', {
        expression: 'document', contextId: world.executionContextId, returnByValue: false,
      });
      const nodeResult = await session.send('Runtime.callFunctionOn', {
        objectId: documentResult.result.objectId,
        functionDeclaration: 'function(selector) { return document.querySelector(selector); }',
        arguments: [{ value: selector }],
        returnByValue: false,
      });
      if (!nodeResult.result.objectId) throw new Error(`Element not found: ${selector}`);
      // Register the complete frame tree in this CDP session before turning
      // the isolated-world object into a DOM node. Without this request,
      // DOM.requestNode can reject a perfectly valid child-frame object.
      await session.send('DOM.getDocument', { depth: -1, pierce: true });
      const { nodeId } = await session.send('DOM.requestNode', { objectId: nodeResult.result.objectId });
      if (nodeId) return { session, nodeId };
      failures.push(`frame ${candidate.id}: DOM.requestNode returned no node id`);
    } catch (err: any) {
      // A cross-process iframe may not be addressable through this page CDP
      // session. The caller records its constrained fallback explicitly.
      failures.push(`frame ${candidate.id}: ${err?.message || String(err)}`);
    }
  }
  throw new Error(`Frame node is unavailable through the page CDP session${failures.length ? ` (${failures.join('; ')})` : ''}`);
}

/**
 * Inspect the selected page or frame. Same-process frames retain the complete
 * CDP cascade and box model. A genuine CDP boundary (for example OOPIF) falls
 * back to verified frame DOM data and marks that limitation in the response.
 */
export async function inspectTargetElement(page: Page, target: Page | Frame, selector: string): Promise<InspectorResult & { inspectionMode: 'cdp' | 'frame-dom-fallback'; inspectionLimitation?: string }> {
  if (target === page) return { ...(await inspectElement(page, selector)), inspectionMode: 'cdp' };
  try {
    const { session, nodeId } = await resolveFrameNodeId(page, target, selector);
    return { ...(await inspectElement(page, selector, { session, nodeId })), inspectionMode: 'cdp' };
  } catch (err: any) {
  const result = await target.evaluate(([selected, properties]) => {
    const element = document.querySelector(selected) as HTMLElement | null;
    if (!element) throw new Error(`Element not found: ${selected}`);
    const style = getComputedStyle(element);
    const rect = element.getBoundingClientRect();
    const attributes: Record<string, string> = {};
    for (const attribute of element.attributes) attributes[attribute.name] = attribute.value;
    const computedStyles: Record<string, string> = {};
    for (const property of properties) computedStyles[property] = style.getPropertyValue(property);
    return {
      tagName: element.tagName.toLowerCase(), id: element.id || null,
      classes: [...element.classList], attributes, computedStyles,
      inlineStyles: Object.fromEntries([...element.style].map((name) => [name, element.style.getPropertyValue(name)])),
      boxModel: (() => {
        const asPx = (value: string) => {
          const parsed = Number.parseFloat(value);
          return Number.isFinite(parsed) ? parsed : 0;
        };
        const padding = { top: asPx(style.paddingTop), right: asPx(style.paddingRight), bottom: asPx(style.paddingBottom), left: asPx(style.paddingLeft) };
        const border = { top: asPx(style.borderTopWidth), right: asPx(style.borderRightWidth), bottom: asPx(style.borderBottomWidth), left: asPx(style.borderLeftWidth) };
        const margin = { top: asPx(style.marginTop), right: asPx(style.marginRight), bottom: asPx(style.marginBottom), left: asPx(style.marginLeft) };
        return {
          content: { x: rect.x + border.left + padding.left, y: rect.y + border.top + padding.top, width: Math.max(0, rect.width - border.left - border.right - padding.left - padding.right), height: Math.max(0, rect.height - border.top - border.bottom - padding.top - padding.bottom) },
          padding, border, margin,
        };
      })(),
    };
  }, [selector, KEY_CSS_PROPERTIES]);
  return {
    selector,
    ...result,
    matchedRules: [],
    pseudoElements: [],
    inspectionMode: 'frame-dom-fallback',
    inspectionLimitation: err?.message || 'Frame CDP inspection was unavailable.',
  };
  }
}
...
export async function modifyStyle(
  ...
  // Inspector server mutations always carry a document token. Keep its check
  // inside the same renderer operation as the write so navigation cannot turn
  // a verified selection into a write on a replacement document.
  if (targetKey && expectedDocumentToken) {
    const result = await page.evaluate(([sel, prop, val, token]) => {
      if (String(performance.timeOrigin) !== token) throw new Error('Inspector target changed. Pick the element again.');
      const element = document.querySelector(sel) as HTMLElement | null;
      if (!element) throw new Error(`Element not found: ${sel}`);
      const oldValue = getComputedStyle(element).getPropertyValue(prop);
      const oldInlineValue = element.style.getPropertyValue(prop);
      const oldInlinePriority = element.style.getPropertyPriority(prop);
      element.style.setProperty(prop, val);
      return { oldValue, oldInlineValue, oldInlinePriority };
    }, [selector, property, value, expectedDocumentToken]);
    const modification: StyleModification = { selector, property, oldValue: result.oldValue, newValue: value, source: 'inline', sourceLine: 0, timestamp: Date.now(), method: 'inline', targetKey, oldInlineValue: result.oldInlineValue, oldInlinePriority: result.oldInlinePriority };
    pushModification(modification);
    return modification;
  }

  if (typeof (page as Page).mainFrame !== 'function') {
    const frame = page as Frame;
    const oldValue = await frame.evaluate(([sel, prop]) => {
      const element = document.querySelector(sel) as HTMLElement | null;
      if (!element) throw new Error(`Element not found: ${sel}`);
      return getComputedStyle(element).getPropertyValue(prop);
    }, [selector, property]);
    await frame.evaluate(([sel, prop, val]) => {
      const element = document.querySelector(sel) as HTMLElement | null;
      if (!element) throw new Error(`Element not found: ${sel}`);
      element.style.setProperty(prop, val);
    }, [selector, property, value]);
    const modification: StyleModification = { selector, property, oldValue, newValue: value, source: 'inline', sourceLine: 0, timestamp: Date.now(), method: 'inline', targetKey };
    pushModification(modification);
    return modification;
  }
  ...
  try {
    ...
    const session = await getOrCreateSession(targetPage);
    const result = await inspectElement(targetPage, selector);
 ...
 */
export async function resetModifications(page: Page | Frame, targetKey?: string, expectedDocumentToken?: string): Promise<void> {
  // Restore in reverse order
  for (let i = modificationHistory.length - 1; i >= 0; i--) {
    const mod = modificationHistory[i];
    if (targetKey !== undefined && mod.targetKey !== targetKey) continue;
    try {
      await page.evaluate(
        ([sel, prop, val, inlineValue, inlinePriority, token]) => {
          if (token && String(performance.timeOrigin) !== token) throw new Error('Inspector target changed. Pick the element again.');
          const el = document.querySelector(sel);
          if (!el) return;
          if (inlineValue !== undefined) {
            if (inlineValue) (el as HTMLElement).style.setProperty(prop, inlineValue, inlinePriority || '');
        ...
        [mod.selector, mod.property, mod.oldValue, mod.oldInlineValue, mod.oldInlinePriority, expectedDocumentToken]
      );
    } catch (err: any) {
  ...
  if (targetKey === undefined) {
    ...
    for (let i = modificationHistory.length - 1; i >= 0; i--) {
      if (modificationHistory[i].targetKey === targetKey) modificationHistory.splice(i, 1);
    }
  }
}
