 ...
 * BFSTACK_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via BFSTACK_HW_CONCURRENCY /
 *      BFSTACK_DEVICE_MEMORY env vars (set by gbd at startup via
 ...
 * on macOS), passed through the BFSTACK_* env vars. Each field falls
...
// the UA-CH cmdline switch in buildbfstackLaunchArgs (which reads BFSTACK_PLATFORM
...
export function readHostProfile(): HostProfile {
  ...
  const concurrency = Number(env.BFSTACK_HW_CONCURRENCY);
  const memory = Number(env.BFSTACK_DEVICE_MEMORY);
  return {
...
export function buildStealthScript(hw: HostProfile): string {
  ...
  // ──── Per-install hardware values from BFSTACK_* env (T2) ────
...
function extendedModeEnabled(): boolean {
  const v = process.env.BFSTACK_STEALTH;
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when BFSTACK_STEALTH=extended (off by
 ...
 * disagree with the env-driven GPU spoof in buildbfstackLaunchArgs on non-Apple
...
/**
 * Build the `--bfstack-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the BFSTACK_* env vars that
 ...
 *
 * TRUSTED-SOURCE ONLY. These BFSTACK_* values are populated by gbd from
 ...
 *   BFSTACK_GPU_VENDOR        → --bfstack-gpu-vendor        → webgl-vendor-spoof.patch
 *   BFSTACK_GPU_RENDERER      → --bfstack-gpu-renderer      → webgl-vendor-spoof.patch
 *   BFSTACK_PLATFORM          → --bfstack-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   BFSTACK_GPU_CHIPSET       → --bfstack-ua-model          → ua-client-hints-stealth.patch
 *   BFSTACK_HW_CONCURRENCY    → --bfstack-hw-concurrency    → worker-navigator-stealth.patch
 *   BFSTACK_DEVICE_MEMORY     → --bfstack-device-memory     → worker-navigator-stealth.patch
 */
export function buildbfstackLaunchArgs(): string[] {
  const env = (globalThis as any).process?.env ?? {};
  const args: string[] = [];

  const vendor = env.BFSTACK_GPU_VENDOR;
  if (vendor) args.push(`--bfstack-gpu-vendor=${vendor}`);

  const renderer = env.BFSTACK_GPU_RENDERER;
  if (renderer) args.push(`--bfstack-gpu-renderer=${renderer}`);
  ...
  const platform = env.BFSTACK_PLATFORM;
  if (platform === 'MacARM' || platform === 'MacIntel') {
    args.push('--bfstack-ua-platform=macOS');
  } else if (platform === 'Win32') {
    args.push('--bfstack-ua-platform=Windows');
  } else if (platform && platform.startsWith('Linux')) {
    args.push('--bfstack-ua-platform=Linux');
  }

  const chipset = env.BFSTACK_GPU_CHIPSET;
  if (chipset) args.push(`--bfstack-ua-model=${chipset}`);

  const hw = env.BFSTACK_HW_CONCURRENCY;
  if (hw) args.push(`--bfstack-hw-concurrency=${hw}`);

  const memory = env.BFSTACK_DEVICE_MEMORY;
  if (memory) args.push(`--bfstack-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when BFSTACK_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // gbrowser builds; gbd opts in by exporting BFSTACK_CDP_STEALTH=on. Stock
  ...
  // unless BFSTACK_CDP_STEALTH=off, which contradicted this very comment.)
  const cdpStealth = env.BFSTACK_CDP_STEALTH;
  if (cdpStealth === 'on' || cdpStealth === '1' || cdpStealth === 'true') {
    args.push('--bfstack-suppress-prepare-stack-trace');
  }
