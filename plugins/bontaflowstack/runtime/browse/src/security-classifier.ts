 ...
 * layout at ~/.bfstack/models/testsavant-small/ on first use.
 *
...
function modelsDir(): string {
  return path.join(resolveBfstackHome(), 'models');
}

function testsavantDir(): string {
  return path.join(modelsDir(), 'testsavant-small');
}
...
async function ensureTestsavantStaged(onProgress?: (msg: string) => void): Promise<void> {
  const modelDir = testsavantDir();
  mkdirSecure(path.join(modelDir, 'onnx'));
  ...
  const modelDst = path.join(modelDir, 'onnx', 'model.onnx');
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.BFSTACK_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'BFSTACK_SECURITY_OFF=1 — ML classifier kill switch engaged';
