...
import { chromium, type Browser, type BrowserContext, type BrowserContextOptions, type Page, type Frame, type Locator, type Cookie } from 'playwright';
...
import { resolveBfstackHome, resolveChromiumProfile, resolveConfig, cleanSingletonLocks } from './config';
...
function chromiumEnvironment() {
  const { stateDir } = resolveConfig();
  mkdirSecure(stateDir);
  return { ...process.env, CHROME_LOG_FILE: path.join(stateDir, 'chromium-debug.log') };
}
...
/**
 * Detect whether BFSTACK_CHROMIUM_PATH points at a custom Chromium build that
 * already bakes the bfstack extension in as a component extension (e.g.,
 * bfstack Browser.app / GBrowser). Passing --load-extension against such a
 * binary triggers a ServiceWorkerState::SetWorkerId DCHECK because two
 * copies of the same service worker try to register.
 *
 * Resolution:
 *   1. BFSTACK_CHROMIUM_KIND === 'custom-extension-baked' (preferred, explicit)
 *   2. BFSTACK_CHROMIUM_PATH path substring contains 'GBrowser' or 'gbrowser'
 *      (fallback for callers that only set the path)
 */
export function isCustomChromium(): boolean {
  if (process.env.BFSTACK_CHROMIUM_KIND === 'custom-extension-baked') return true;
  const p = process.env.BFSTACK_CHROMIUM_PATH || '';
  return p.includes('GBrowser') || p.includes('gbrowser');
}

/**
 * Resolve Chromium from the selected bfstack runtime only. A packaged browser
 * must never fall back to a developer's Playwright cache or a personal browser
 * profile, because that would make a prebuilt Windows installation depend on
 * machine-local development state.
 */
export function resolveBundledChromiumPath(env: NodeJS.ProcessEnv = process.env): string {
  const runtimeRoot = env.BFSTACK_ROOT;
  if (!runtimeRoot) throw new Error('BFSTACK_ROOT is required to resolve the packaged Chromium runtime.');
  const browsersRoot = path.resolve(runtimeRoot, 'browse', '.playwright-browsers');
  const explicit = env.BFSTACK_CHROMIUM_PATH;
  if (explicit) {
    if (!fs.existsSync(explicit)) throw new Error(`BFSTACK_CHROMIUM_PATH does not exist: ${explicit}`);
    const relative = path.relative(browsersRoot, path.resolve(explicit));
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new Error('BFSTACK_CHROMIUM_PATH must remain inside the packaged Chromium runtime.');
    }
    return explicit;
  }

  let revisions: string[];
  try { revisions = fs.readdirSync(browsersRoot).filter((name) => /^chromium-\d+$/.test(name)).sort().reverse(); }
  catch { throw new Error(`Packaged Chromium directory is unavailable: ${browsersRoot}`); }
  for (const revision of revisions) {
    for (const directory of ['chrome-win64', 'chrome-win']) {
      const candidate = path.join(browsersRoot, revision, directory, 'chrome.exe');
      if (fs.existsSync(candidate)) return candidate;
    }
  }
  throw new Error(`Packaged Chromium executable is unavailable below: ${browsersRoot}`);
}
...
export function shouldEnableChromiumSandbox(): boolean {
  ...
  // so /qa hangs without --no-sandbox). Setting BFSTACK_CHROMIUM_NO_SANDBOX=1
  // forces the sandbox off without changing the default for everyone else.
  // See #1562.
  if (process.env.BFSTACK_CHROMIUM_NO_SANDBOX === '1') return false;
 ...
 * (chromium.executablePath()). A bundle supplied via BFSTACK_CHROMIUM_PATH
 * belongs to the wrapper/embedder — its plist legitimately says "bfstack
 ...
 * BFSTACK_CHROMIUM_PATH executable itself.
 *
...
export function probePoisonedChromiumBundle(chromiumExecutablePath: string): void {
  ...
  const customPath = process.env.BFSTACK_CHROMIUM_PATH;
  ...
  if (!fs.readFileSync(chromePlist, 'utf-8').includes('bfstack Browser')) return;
  ...
  throw new PoisonedBundleError(
    'Chromium bundle was mutated by a previous bfstack version (broken codesign seal — ' +
...
export async function handleChromiumDisconnect(browser: Browser | null): Promise<void> {
  ...
  console.error('[browse] Console/network logs flushed to .bfstack/browse-*.log');
  process.exit(1);
}
...
export interface InspectorTarget {
  /** Server-local tab id. Chrome extension tab ids are deliberately excluded. */
  serverTabId?: number;
  /** URL of the main document at selection time. */
  documentUrl: string;
  /** Browser document identity from performance.timeOrigin; disambiguates duplicate URLs. */
  documentToken?: string;
  /** Selected child frame, when the pick happened inside one. */
  frameUrl?: string;
  frameName?: string | null;
  frameDocumentToken?: string;
  /** Main-document generation returned by an earlier pick. */
  generation?: number;
}
...
export interface ResolvedInspectorTarget {
  page: Page;
  frame: Page | Frame;
  target: Required<Pick<InspectorTarget, 'serverTabId' | 'documentUrl' | 'documentToken' | 'generation'>> & Pick<InspectorTarget, 'frameUrl' | 'frameName' | 'frameDocumentToken'>;
}

/** A caller-visible target problem. The HTTP layer maps this to its safe status. */
export class InspectorTargetError extends Error {
...
export class BrowserManager {
  ...
  /** Main-document generation, incremented on every navigation for stale target rejection. */
  private pageGenerations: WeakMap<Page, number> = new WeakMap();
  ...
  /** Server port — set after server starts for the local extension handoff. */
  ...
  /**
   * Find the bfstack Chrome extension directory.
   * The extension is part of the selected bfstack runtime. Do not discover a
   * developer checkout or a profile-installed extension at runtime.
   */
  private findExtensionPath(): string | null {
    const fs = require('fs');
    const path = require('path');
    const runtimeRoot = process.env.BFSTACK_ROOT;
    if (!runtimeRoot) return null;
    const candidates = [path.join(runtimeRoot, 'extension')];
  ...
  async launch() {
    ...
    const { STEALTH_LAUNCH_ARGS, buildbfstackLaunchArgs } = await import('./stealth');
    const launchArgs: string[] = [...STEALTH_LAUNCH_ARGS, ...buildbfstackLaunchArgs()];
    ...
    if (extensionsDir) {
      ...
      // already bakes the extension in (e.g., GBrowser / bfstack Browser.app).
    ...
    const executablePath = resolveBundledChromiumPath();
    ...
    // bundled executable, so the heal remains scoped out from package assets.
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the bfstack
   ...
   * every action the agent takes in real time.
   */
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the bfstack extension directory for auto-loading
    ...
    const launchArgs = [
      ...
      ...STEALTH_LAUNCH_ARGS,
      // bfstack Pack 1: per-install hardware/GPU/UA-CH overrides for the
    ...
    if (extensionPath) {
      ...
      // (gbrowser / bfstack Browser.app). Loading it twice causes a
      ...
      // the extension is component-baked — it reads ~/.bfstack/.auth.json at
      // startup to learn how to call the daemon).
      // Write to ~/.bfstack/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const bfstackDir = resolveBfstackHome();
        mkdirSecure(bfstackDir);
        const authFile = path.join(bfstackDir, '.auth.json');
        try {
    ...
    // single-instance CLI check for bfstack.
    cleanSingletonLocks(userDataDir);

    // Support custom Chromium binary via BFSTACK_CHROMIUM_PATH env var.
    // Used by bfstack Browser.app to point at the bundled Chromium.
    const executablePath = resolveBundledChromiumPath();

    // NOTE (#2242): the in-place "rebrand" that patched the Chromium .app's
    // Info.plist (global "Google Chrome for Testing" → "bfstack Browser"
    ...
    // bfstack Browser.app wrapper (BFSTACK_CHROMIUM_PATH), never in a mutation
    ...
    // BFSTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
    // never probed.
    // Use Chromium's native UA unless explicitly overridden. Running chrome.exe
    // just for --version on Windows can start a timed-out crashpad process.
    ...
    // heal out when BFSTACK_CHROMIUM_PATH supplies the bundle — that bundle
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('bfstack-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'bfstack-ctrl';
        topLine.style.cssText = `
          position: fixed; top: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
          background-size: 200% 100%;
          animation: bfstack-shimmer 3s linear infinite;
        ...
        style.textContent = `
          @keyframes bfstack-shimmer {
            0% { background-position: 200% 0; }
            100% { background-position: -200% 0; }
          }
          @media (prefers-reduced-motion: reduce) {
            #bfstack-ctrl { animation: none !important; }
          }
        `;
  ...
  // ─── Tab Management ────────────────────────────────────────
  private registerPage(page: Page, clientId?: string): number {
    // A headed newPage() also emits 'page' before its promise resolves.
    for (const [id, registered] of this.pages) {
      if (registered !== page) continue;
      if (clientId) this.tabOwnership.set(id, clientId);
      return id;
    }
  ...
  async newTab(url?: string, clientId?: string): Promise<number> {
    ...
    const id = this.registerPage(page, clientId);
    this.activeTabId = id;
   ...
   *
   *   - **own-only:** the strict mode. Token must own
   *     the target tab for any access (reads or writes). Unowned user tabs
   *     and tabs owned by other clients are off-limits. Scoped clients must
   ...
   *     elsewhere (scope checks at /command and rate limits). Tab ownership
   *     limits access only for own-only tokens.
  ...
  /**
   * Resolve an Inspector request without consulting the mutable active tab.
   * A URL-only target is accepted only when it identifies one tracked page;
   * that makes duplicate URLs a visible conflict instead of a wrong-page read.
   */
  async resolveInspectorTarget(request: InspectorTarget): Promise<ResolvedInspectorTarget> {
    if (!request || !request.documentUrl) {
      throw new InspectorTargetError(400, 'Inspector target documentUrl is required. Pick the element again.');
    }

    let candidates: Array<[number, Page]>;
    if (request.serverTabId !== undefined) {
      const page = this.pages.get(request.serverTabId);
      if (!page || page.isClosed()) {
        throw new InspectorTargetError(404, 'The selected Inspector tab no longer exists. Pick the element again.');
      }
      candidates = [[request.serverTabId, page]];
    } else {
      candidates = [...this.pages.entries()].filter(([, page]) => !page.isClosed() && page.url() === request.documentUrl);
      if (candidates.length === 0) {
        throw new InspectorTargetError(404, 'The selected Inspector document no longer exists. Pick the element again.');
      }
      if (request.documentToken) {
        const matches: Array<[number, Page]> = [];
        for (const candidate of candidates) {
          const token = await candidate[1].evaluate(() => String(performance.timeOrigin)).catch(() => null);
          if (token === request.documentToken) matches.push(candidate);
        }
        candidates = matches;
      }
      // Content scripts in child frames cannot read the top document's
      // timeOrigin. The selected frame's document token is still a stable
      // browser identity, so use it to disambiguate same-URL parent pages
      // before declaring the target ambiguous. Chrome tab ids never enter
      // this mapping: they are unrelated to these server-local tab ids.
      if (!request.documentToken && request.frameUrl && request.frameDocumentToken) {
        const matches: Array<[number, Page]> = [];
        for (const candidate of candidates) {
          const frames = candidate[1].frames().filter((frame) =>
            frame !== candidate[1].mainFrame()
            && !frame.isDetached()
            && frame.url() === request.frameUrl
            && (request.frameName === undefined || request.frameName === null || frame.name() === request.frameName)
          );
          const tokenMatches = await Promise.all(frames.map(async (frame) =>
            (await frame.evaluate(() => String(performance.timeOrigin)).catch(() => null)) === request.frameDocumentToken
          ));
          if (tokenMatches.some(Boolean)) matches.push(candidate);
        }
        candidates = matches;
      }
      if (candidates.length === 0) {
        throw new InspectorTargetError(409, 'The selected Inspector document changed. Pick the element again.');
      }
      if (candidates.length > 1) {
        throw new InspectorTargetError(409, 'The selected Inspector document is ambiguous. Pick the element again.');
      }
    }

    const [serverTabId, page] = candidates[0];
    if (page.url() !== request.documentUrl) {
      throw new InspectorTargetError(409, 'The selected Inspector document changed. Pick the element again.');
    }
    const generation = this.pageGenerations.get(page) ?? 0;
    if (request.generation !== undefined && request.generation !== generation) {
      throw new InspectorTargetError(409, 'The selected Inspector document changed. Pick the element again.');
    }

    const documentToken = await page.evaluate(() => String(performance.timeOrigin)).catch(() => null);
    if (!documentToken || (request.documentToken && request.documentToken !== documentToken)) {
      throw new InspectorTargetError(409, 'The selected Inspector document changed. Pick the element again.');
    }

    let frame: Page | Frame = page;
    let frameDocumentToken: string | undefined;
    if (request.frameUrl) {
      let frames = page.frames().filter((candidate) =>
        candidate !== page.mainFrame()
        && !candidate.isDetached()
        && candidate.url() === request.frameUrl
        && (request.frameName === undefined || request.frameName === null || candidate.name() === request.frameName)
      );
      if (request.frameDocumentToken) {
        const tokenMatches = await Promise.all(frames.map(async (candidate) =>
          (await candidate.evaluate(() => String(performance.timeOrigin)).catch(() => null)) === request.frameDocumentToken
        ));
        frames = frames.filter((_, index) => tokenMatches[index]);
      }
      if (frames.length === 0) {
        throw new InspectorTargetError(404, 'The selected Inspector frame no longer exists. Pick the element again.');
      }
      if (frames.length > 1) {
        throw new InspectorTargetError(409, 'The selected Inspector frame is ambiguous. Pick the element again.');
      }
      frame = frames[0];
      frameDocumentToken = await frame.evaluate(() => String(performance.timeOrigin)).catch(() => undefined);
      if (!frameDocumentToken || (request.frameDocumentToken && request.frameDocumentToken !== frameDocumentToken)) {
        throw new InspectorTargetError(409, 'The selected Inspector frame changed. Pick the element again.');
      }
    }
    ...
    return {
      ...
      target: {
        ...
        documentToken,
        generation,
        ...(request.frameUrl ? { frameUrl: request.frameUrl, frameName: request.frameName ?? null, frameDocumentToken } : {}),
      },
    };
  }
  ...
  async restoreState(state: BrowserState): Promise<void> {
    ...
    for (const saved of state.pages) {
      const page = await this.context.newPage();
      const id = this.registerPage(page, saved.owner);
      const newSession = this.tabSessions.get(id)!;
  ...
  async setDeviceScaleFactor(scale: number, width: number, height: number): Promise<string | null> {
    ...
    if (scale < 1 || scale > 3) {
      throw new Error(`viewport --scale: value must be between 1 and 3 (bfstack policy cap), got ${scale}`);
    }
    if (this.connectionMode === 'headed') {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      const launchArgs: string[] = ['--hide-crash-restore-bubble', ...STEALTH_LAUNCH_ARGS, ...buildbfstackLaunchArgs()];
      if (extensionPath) {
        launchArgs.push(`--disable-extensions-except=${extensionPath}`);
        launchArgs.push(`--load-extension=${extensionPath}`);
      ...
      // This path previously hardcoded ~/.bfstack/chromium-profile, silently
      // ignoring $CHROMIUM_PROFILE / $BFSTACK_HOME and skipping the lock
      ...
      const { STEALTH_IGNORE_DEFAULT_ARGS } = await import('./stealth');
      // Package assets are never eligible for an in-place cache self-heal.
  ...
  private wirePageEvents(page: Page) {
    this.pageGenerations.set(page, this.pageGenerations.get(page) ?? 0);
    ...
    page.on('framenavigated', (frame) => {
      if (frame === page.mainFrame()) {
        this.pageGenerations.set(page, (this.pageGenerations.get(page) ?? 0) + 1);
