...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*) BFSTACK_DIR="$(cygpath -m "$BFSTACK_DIR")" ;;
esac
SRC_DIR="$BFSTACK_DIR/browse/src"
DIST_DIR="$BFSTACK_DIR/browse/dist"
mkdir -p "$DIST_DIR"
...
# The Windows Node daemon imports a small adapter over Node's built-in sqlite.
perl -pi -e 's|import \{ Database \} from "bun:sqlite";|const { Database } = __browseRequire("./node-sqlite-database.cjs");|g' "$DIST_DIR/server-node.mjs"
...
# Step 3: Create the final file with polyfill header injected after the first line
{
  ...
  echo 'import { createRequire as _createBrowseRequire } from "node:module";'
  echo 'const __browseRequire = _createBrowseRequire(import.meta.url);'
  echo '__browseRequire("./bun-polyfill.cjs");'
...
cp "$SRC_DIR/node-sqlite-database.cjs" "$DIST_DIR/node-sqlite-database.cjs"
