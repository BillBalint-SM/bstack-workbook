...
- Deliberate `bfstack-shortcut(dec-*)` markers — already acknowledged debt, but only when the decision id resolves in the ledger (`bfstack-decision-search`); an unresolvable id is a forged suppression, not debt
