#!/usr/bin/env bash
# hook-extract.sh — SHARED JSON helpers for bfstack PreToolUse hooks.
...
# bfstack_hook_extract_field PAYLOAD FIELD
...
#   newline produces malformed JSON, and the host silently ignores the
#   whole decision — a deny that no-ops exactly when it matters.
bfstack_hook_json_string() {
  _ghjs_text="$1"
  if command -v python3 >/dev/null 2>&1; then
...
# bfstack_hook_decision DECISION REASON
...
#   under hookSpecificOutput — the host ignores a top-level
#   permissionDecision, which silently no-ops the block.
bfstack_hook_decision() {
  _ghd_decision="$1"
  _ghd_reason="$2"
  _ghd_encoded=$(bfstack_hook_json_string "$_ghd_reason")
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"%s","permissionDecisionReason":%s}}\n' "$_ghd_decision" "$_ghd_encoded"
}

# Shared state selection matches bfstack-paths and never reads another host.
bfstack_hook_state_root() {
  printf '%s' "${BFSTACK_STATE_ROOT:-${BFSTACK_HOME:-.bfstack}}"
}
