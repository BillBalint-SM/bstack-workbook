...
# The decision MUST be nested under hookSpecificOutput — the host ignores a
...
# unreadable input.
set +e
CMD=$(bfstack_hook_extract_field "$INPUT" command)
EXTRACT_RC=$?
set -e

# No parser available, or the payload is not parseable JSON. Fail closed.
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  bfstack_hook_decision ask "[careful] Could not parse the tool payload to safety-check this command. Approve only if you know what it does."
  exit 0
fi
...
if printf '%s' "$CMD" | grep -qE '\$\{IFS\}|\$IFS|\$\(echo[^)]*base64[^)]*\)|base64[[:space:]]+(-d|--decode)[^|]*\|[[:space:]]*(sh|bash)' 2>/dev/null; then
  bfstack_hook_decision ask "[careful] Shell obfuscation detected (IFS word-splitting or base64-to-shell). Read the command carefully before approving."
  exit 0
fi
...
# installer flows, including bfstack's own setup pattern.
_IS_SIMPLE=1
case "$CMD" in
  *';'*|*'&&'*|*'||'*|*'|'*|*$'\n'*) _IS_SIMPLE=0 ;;
esac
if [ "$_IS_SIMPLE" -eq 1 ]; then
  ...
  if printf '%s' "$CMD" | grep -qE '^[[:space:]]*(sudo[[:space:]]+)?rm[[:space:]]' 2>/dev/null \
    ...
    if [ "$_ROOT_TARGETS" -eq 1 ] && [ "$_SAFE_TARGETS" -eq 0 ]; then
      bfstack_hook_decision deny "[careful][HIGH] Recursive delete of / or the home directory is blocked while /careful is active. If you truly mean it, end the /careful session first."
      exit 0
    fi
  fi
    ...
    if [ "$_HAS_FORCE" -eq 1 ]; then
      ...
      if [ -n "$_DEFAULT_BRANCH" ]; then
        ...
        if [ "$_TARGETS_DEFAULT" -eq 1 ]; then
          bfstack_hook_decision deny "[careful][HIGH] Force-push to the default branch ($_DEFAULT_BRANCH) is blocked while /careful is active. Use --force-with-lease on a feature branch, or end the /careful session if you truly mean it."
          exit 0
        fi
      fi
    fi
...
if [ -z "$WARN" ] && printf '%s' "$CMD" | grep -qE 'git[[:space:]]+push[^;&|]*([[:space:]](-f|--force)([[:space:]]|$)|[[:space:]]+\+[^[:space:];&|]+)' 2>/dev/null; then
...
if [ -z "$WARN" ]; then
  DIAGNOSTICS=""
  # Same state root the writer (/careful via bfstack-paths) uses — see
  # bfstack_hook_state_root in hook-extract.sh (#1459 class).
  if command -v bfstack_hook_state_root >/dev/null 2>&1; then
    _BFSTACK_HOME_DIR="$(bfstack_hook_state_root; printf x)"; _BFSTACK_HOME_DIR="${_BFSTACK_HOME_DIR%x}"
  else
    # Older hook-extract.sh (partial upgrade): the plain chain beats dying
    # under set -e with no decision JSON — rules under $HOME/.bfstack still load.
    _BFSTACK_HOME_DIR="${BFSTACK_HOME:-$HOME/.bfstack}"
  fi
  _PATTERN_FILES="$_BFSTACK_HOME_DIR/careful-patterns.txt"
  ...
  _ANY_PROJ_PAT=$(find "$_BFSTACK_HOME_DIR/projects" -maxdepth 2 -name careful-patterns.txt -print -quit 2>/dev/null || true)
  if [ -n "$_ANY_PROJ_PAT" ]; then
    eval "$("$_HOOK_DIR/../../bin/bfstack-slug" 2>/dev/null)" 2>/dev/null || true
    if [ -n "${SLUG:-}" ]; then
      _PATTERN_FILES="$_PATTERN_FILES
$_BFSTACK_HOME_DIR/projects/$SLUG/careful-patterns.txt"
    fi
  fi
  while IFS= read -r _PF; do
    ...
    while IFS= read -r _PAT || [ -n "$_PAT" ]; do
      ...
      _RULE="$_PAT"
      case "$_RULE" in warn:[[:space:]]*) _RULE="${_RULE#warn: }" ;; esac
      _PAT_RC=0
      printf '' | grep -qE -- "$_RULE" 2>/dev/null || _PAT_RC=$?
      if [ "$_PAT_RC" -eq 2 ]; then
        DIAGNOSTICS="${DIAGNOSTICS}${DIAGNOSTICS:+; }Invalid careful rule: $_PAT"
        continue
      fi
      if printf '%s' "$CMD" | grep -qE -- "$_RULE" 2>/dev/null; then
        WARN="Project rule matched: $_RULE"
...
# --- Output ---
if [ -n "$WARN" ]; then
  bfstack_hook_decision ask "[careful] $WARN${DIAGNOSTICS:+; $DIAGNOSTICS}"
elif [ -n "${DIAGNOSTICS:-}" ]; then
  bfstack_hook_decision ask "[careful] $DIAGNOSTICS"
else
  echo '{}'
fi
