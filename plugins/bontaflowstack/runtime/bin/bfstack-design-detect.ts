#!/usr/bin/env bun
/**
 * bfstack-design-detect — find, and run, an impeccable engine the USER installed.
 *
 *   bun --no-env-file run $BFSTACK_BIN/bfstack-design-detect.ts probe [--host <h>] [--verbose]
 *   bun --no-env-file run $BFSTACK_BIN/bfstack-design-detect.ts scan [--format bfstack|raw] [--changed <base>] [--host <h>] <paths...>
 *   bun --no-env-file run $BFSTACK_BIN/bfstack-design-detect.ts rules
 *   bun --no-env-file run $BFSTACK_BIN/bfstack-design-detect.ts install [--version <v>] [--sha256 <hex>] [--base <url>]
 *
 * Rule zero: bfstack never runs impeccable's installer, its launcher
 ...
 * JSON parsing. The one download bfstack itself can make is `install`: the engine
 * binary for a version bfstack has tested, fetched only after the user said yes
 ...
 *          │
 *   ~/{.codex,.agents}/skills/impeccable/scripts/
 ...
 * checked-out branch can commit `.agents/skills/impeccable/scripts/bin/<os>-<arch>/
 ...
 * (--format bfstack) or the engine's own bytes (--format raw); everything else
 * goes to stderr, matching impeccable's own split. Exit code passes through
 * (1 over 2 over 0); exit 3 is a bfstack bug (DESIGN_DETECT_INTERNAL_ERROR).
 *
 ...
 * cwd) or under ${BFSTACK_HOME:-~/.bfstack}/projects/<slug>/designs/ (where design-
 ...
 * page-controlled; other designs/ files are bfstack-authored artifacts and keep
...
/** Where config.yaml lives: the same precedence bin/bfstack-config uses. */
function bfstackStateDir(): string {
  return ENV.BFSTACK_STATE_ROOT || ENV.BFSTACK_HOME || ENV.BFSTACK_STATE_DIR || path.join(HOME, '.bfstack');
}

/** Where projects/<slug>/designs/ lives: the `${BFSTACK_HOME:-$HOME/.bfstack}` rule the skill templates and bfstack-slug render. */
function bfstackHome(): string {
  return ENV.BFSTACK_HOME || path.join(HOME, '.bfstack');
}
...
/** One flat key from config.yaml, read the way bin/bfstack-config resolves it (same STATE_DIR precedence); '' when unset. */
function configValue(key: string): string {
  const file = path.join(bfstackStateDir(), 'config.yaml');
  try {
...
const HOSTS_WITH_HOOKS: Record<string, string[]> = { codex: ['.codex/hooks.json'] };
const SKILL_ROOTS = ['.codex', '.agents'];
  ...
  if (launcher) {
    ...
    const how = p.launcher
      ...
      : repoLocalLauncher && !launcherOnPath
        ? 'the skill is installed inside this repository, and bfstack never runs a repository-local launcher; install it under your home directory (`npx impeccable install --scope global` outside the repo) if you want the engine here'
    ...
    p.notes.push(`${SENTINEL.HINT}: impeccable is installed but its engine is not cached; ${how}, or accept the install offer. Silence this: \`bfstack-config set design_detector off\`.`);
...
// ── Install (the one download bfstack makes, after consent) ───────────────────
 ...
 * .sha256 sidecar for a version bfstack has not pinned; --base allows a mirror
 * (https, or http on loopback for tests).
 */
async function install(args: InstallArgs): Promise<number> {
  if (configDesignDetector() === 'off') return installRefused('design_detector is off; `bfstack-config set design_detector auto` first');
  const platform = enginePlatform();
  ...
  if (!expected) return installRefused(`bfstack pins no checksum for engine ${version} on ${platform}; pass --sha256 <hex> from the release's .sha256 sidecar to accept an unpinned download`);
  ...
  try {
    receipt = writeReceipt({
      ...
      consent: `design_detector=auto design_detector_install_prompted=false; user accepted the install offer for impeccable engine ${version} (${platform}); checksum ${args.sha256 ? 'from --sha256' : 'pinned in bfstack'}`,
    }).id;
  } catch (err) {
...
interface ScanArgs { format: 'bfstack' | 'raw'; changed?: string; targets: string[]; host: string }
...
function designsRoot(): string {
  return path.join(bfstackHome(), 'projects');
}
 ...
 *   dom-dump  <bfstack home>/projects/<slug>/designs/<audit>/dom/**  (a page's own bytes)
 *   artifact  any other file under <bfstack home>/projects/<slug>/designs/ (bfstack-authored: finalized.html, previews)
 */
...
function scan(args: ScanArgs): number {
  ...
  const timeoutMs = Number(ENV.BFSTACK_DESIGN_DETECT_TIMEOUT_MS) > 0 ? Number(ENV.BFSTACK_DESIGN_DETECT_TIMEOUT_MS) : DETECT_LIMITS.timeoutMs;
  ...
  // Repository files and bfstack-authored artifacts honor inline `impeccable-disable`
...
function parse(argv: string[]): { verb: string; host: string; verbose: boolean; scan: ScanArgs; install: InstallArgs } {
  ...
  let host = 'codex';
  let verbose = false;
  let format: 'bfstack' | 'raw' = 'bfstack';
  ...
  for (let i = 1; i < argv.length; i++) {
    ...
    else if (a === '--format') { const v = argv[++i]; format = v === 'raw' ? 'raw' : 'bfstack'; }
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  switch (verb) {
    ...
    default:
      process.stderr.write('usage: bfstack-design-detect.ts probe [--host <h>] [--verbose] | scan [--format bfstack|raw] [--changed <base>] [--host <h>] <paths...> | rules | install [--version <v>] [--sha256 <hex>] [--base <url>]\n');
      return 2;
  }
}
