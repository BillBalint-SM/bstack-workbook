# bfstack-egress-lib.sh — shared egress-receipt helpers for bash sinks.
#
# This file is NOT executable; source it:
#
#   . "$(dirname "$0")/bfstack-egress-lib.sh"
#
...
#     the receipt id is exported as _BFSTACK_EGRESS_LAST_RECEIPT so callers
...
case "${BASH_SOURCE[0]}" in
  */*) _bfstack_egress_lib_dir="$(cd "${BASH_SOURCE[0]%/*}" && pwd)" ;;
  *) _bfstack_egress_lib_dir="$(pwd)" ;;
esac

_bfstack_egress_home() {
  if [ -n "${BFSTACK_HOME:-}" ]; then
    printf '%s' "$BFSTACK_HOME"
  elif [ -n "${BFSTACK_STATE_DIR:-}" ]; then
    printf '%s' "$BFSTACK_STATE_DIR"
  else
    printf '%s' "$HOME/.bfstack"
  fi
}

# Problem + cause + fix, in plain language (DX contract for every
# fail-closed refusal). $1 = sink, $2 = cause text from the receipt bridge.
_bfstack_egress_refusal() {
  local home
  home="$(_bfstack_egress_home)"
  echo "bfstack: $1 NOT sent — the egress receipt could not be written (${2:-unknown cause}). Fix: chmod -R u+w $home/security (or check BFSTACK_HOME). What this is: bfstack records everything it ATTEMPTS to send off-machine; see bfstack-egress." >&2
}
...
_bfstack_egress_run() {
  ...
  shift 6
  local bin="$_bfstack_egress_lib_dir/bfstack-egress-receipt"

  local payload_flag=(--no-payload)
  [ "$payload" != "--no-payload" ] && payload_flag=(--payload-file "$payload")

  local receipt_id="" receipt_err="" err_file=""
  err_file="$(mktemp "${TMPDIR:-/tmp}/bfstack-egress-err-XXXXXX" 2>/dev/null)" || err_file=""
  if [ -n "$err_file" ]; then
  ...
  _BFSTACK_EGRESS_LAST_RECEIPT="$receipt_id"

  if [ -z "$receipt_id" ]; then
    if [ "$policy" = "closed" ]; then
      _bfstack_egress_refusal "$sink" "$receipt_err"
      [ "$payload" != "--no-payload" ] && rm -f "$payload"
      return 3
    fi
    echo "bfstack: egress receipt could not be written for $sink (${receipt_err:-unknown cause}) — sending anyway (fail-open). bfstack normally records everything it ATTEMPTS to send off-machine; see bfstack-egress." >&2
  fi
...
_receipted_git() {
  ...
  shift 5
  _bfstack_egress_run "$policy" "$sink" "$host" "$class" "$consent" --no-payload "$@"
}
