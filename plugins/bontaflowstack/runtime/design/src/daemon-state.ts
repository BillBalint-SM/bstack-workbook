...
export const CMDLINE_MARKER = "bfstack-design-daemon";
...
export function resolveStateFilePath(): string {
  ...
  try {
    ...
    if (root) return path.join(root, ".bfstack", "design.json");
  } catch {
    // not in a git repo — fall through
  }
  return path.join(process.cwd(), ".bfstack", "design.json");
}
...
export function resolveDaemonLogPath(): string {
  return path.join(resolveDesignStateRoot(), "design-daemon.log");
}

export function resolveStartupLogPath(): string {
  return path.join(resolveDesignStateRoot(), "design-daemon-startup.log");
}

function resolveDesignStateRoot(): string {
  return process.env.BFSTACK_HOME || process.env.BFSTACK_STATE_ROOT || process.env.BFSTACK_STATE_DIR || path.join(os.homedir(), ".bfstack");
}

/**
 * Read the bfstack version both client and daemon should agree on. Looks
