/**
 * Question Registry — typed schema for AskUserQuestion invocations across bfstack.
 *
 * Purpose
 * -------
 ...
 *
 * Not every AskUserQuestion in bfstack needs a registry entry right away. Skills
...
export interface QuestionDef {
  ...
  id: string;
  /** Skill that owns this question (must match a bfstack skill directory name) */
  skill: string;
  ...
  signal_key?: string;
  /** Explicitly safe for an advisory exact-choice preference. Defaults to false. */
  optional_preference?: boolean;
...
export const QUESTIONS = {
  ...
  'land-and-deploy-rollback': {
    ...
    options: ['accept', 'reject'],
    signal_key: 'decision-autonomy',
    description: "The deploy health check found regressions — roll back the deploy?",
  },
  ...
  'preamble-proactive-behavior': {
    ...
    door_type: 'two-way',
    options: ['on', 'off'],
    description: "Let bfstack proactively suggest skills based on conversation context?",
  },
  'preamble-routing-injection': {
    ...
    description: "Add bfstack skill routing rules to AGENTS.md?",
  },
  'preamble-vendored-migration': {
    ...
    description: "This repo has vendored bfstack (deprecated) — migrate to team mode?",
  },
...
/** Only explicitly marked low-impact routing questions may have preferences. */
export function isOptionalPreferenceEligible(id: string): boolean {
  return getQuestion(id)?.optional_preference === true;
}
