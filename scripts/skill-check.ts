...
const primaryGeneration = getHostConfig('claude').generation;

for (const { tmpl, output } of TEMPLATES) {
  const tmplPath = path.join(ROOT, tmpl);
  // Root outputs belong to the primary host; mirror the generator's selection.
  const skill = path.basename(path.dirname(tmplPath));
  if (primaryGeneration.includeSkills?.length && !primaryGeneration.includeSkills.includes(skill)) continue;
  if (primaryGeneration.skipSkills?.includes(skill)) continue;
...
try {
  execSync('bun scripts/build-bstack-plugin.ts --check', { cwd: ROOT, stdio: 'pipe' });
  console.log('  bstack plugin wrappers: fresh');
} catch (err: any) {
  hasErrors = true;
  console.log('  bstack plugin wrappers: FAILED');
