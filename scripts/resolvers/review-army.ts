 ...
 *
 * Generates template prose that instructs the host to:
...
${ctx.host === 'codex' ? `Use the available, authorized Codex subagent tools. If independent subagents
are unavailable or not authorized, report the limitation and stop this stage;
do not substitute a self-review or a new user-visible task.

` : ''}For each selected specialist, launch an independent subagent via ${ctx.host === 'codex' ? 'the Codex subagent tool' : 'the Agent tool'}.
**Launch ALL selected specialists in a single message** (multiple ${ctx.host === 'codex' ? 'Codex subagent' : 'Agent'} tool calls)
...
${ctx.host === 'codex' ? `- Use the host's documented spawn arguments; do not pass Claude-only options.
- Keep reviewer IDs and wait for ALL selected reviewers to finish before merging.
- Reviewers are read-only and must not delegate, edit, or approve user decisions.` : `- Use \`subagent_type: "general-purpose"\`
...
${ctx.host === 'codex' ? 'If activated, dispatch one independent, read-only reviewer via the available Codex subagent tool and wait for its result before continuing. Apply the same authorization and availability requirements as specialist dispatch.' : `If activated, dispatch one more subagent via the Agent tool (pass \`run_in_background: false\` — foreground; subagents default to background since ${CC_BACKGROUND_DEFAULT_SINCE}).`}
